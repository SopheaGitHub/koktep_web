...
class Rating extends Model
{
	...
	public function getTotalRatingPostByPostId($rating_of_id, $rating_type_id) {
		$result = DB::table('rating')->select(DB::raw('CEIL((SUM(star) / COUNT(1))) AS average_rating, SUM(star) AS total_rating'))->where('rating_of_id', '=', $rating_of_id)->where('rating_type_id', '=', $rating_type_id)->first();
		return $result;
	}
